...
export default async function AnalyticsPage() {
  ...
  const [{ data: recentMessages }, { data: recipientRows }, { data: adContacts }, { data: orders }] =
    await Promise.all([
      supabase
        .from("messages")
  ...
  for (const o of orders ?? []) {
    ...
    const bucket = byCampaignRevenue.get(o.attributed_campaign_id) ?? {
      name: campaign.name,
      revenue: 0,
      orderCount: 0,
    };
  ...
  const replyRate =
    totalOutbound > 0 && totalInbound > 0 ? Math.round((totalInbound / totalOutbound) * 100) : null;
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="sk-pill text-faint">Last {DAYS} days</div>
      </div>

      {/* ── Summary tiles ── */}
      <div className="sk-card mb-6 flex">
        <div className="flex-1 border-r border-border px-5 py-4">
          ...
          <div className="mt-0.5 text-[11px] text-faint">messages out</div>
        </div>
        <div className="flex-1 border-r border-border px-5 py-4">
          <div className="mb-1.5 text-[11px] font-medium uppercase tracking-wide text-faint">Received</div>
          <div className="font-mono text-[22px] font-semibold tabular-nums">{totalInbound.toLocaleString()}</div>
          <div className="mt-0.5 text-[11px] text-faint">inbound replies</div>
        </div>
        <div className="flex-1 border-r border-border px-5 py-4">
          <div className="mb-1.5 text-[11px] font-medium uppercase tracking-wide text-faint">Reply rate</div>
          <div
            className="font-mono text-[22px] font-semibold tabular-nums"
            style={{ color: replyRate !== null ? "var(--accent)" : "var(--faint)" }}
          >
            {replyRate !== null ? `${replyRate}%` : "—"}
          </div>
          <div className="mt-0.5 text-[11px] text-faint">customers responded</div>
        </div>
      ...
      <div className="sk-card mb-6 overflow-hidden">
        ...
        <div className="px-4 pb-4 pt-4">
              ...
              return (
                <div key={key} className="group relative flex flex-1 flex-col items-center justify-end gap-px">
                  <div
                    ...
                    style={{ height: `${inboundH}%`, background: "var(--accent-dim)", minHeight: v.inbound ? 2 : 0 }}
                  />
                  <div
                    className="w-full rounded-t-sm"
                    style={{ height: `${outboundH}%`, background: "var(--accent)", minHeight: v.outbound ? 2 : 0 }}
                  />
                  <div className="pointer-events-none absolute bottom-full z-10 mb-1.5 hidden whitespace-nowrap rounded border border-border bg-[var(--surface-2)] px-2 py-1.5 text-[11px] shadow-sm group-hover:block">
                    ...
                    <div className="text-faint">
                      {v.outbound} sent · {v.inbound} received
                    </div>
                  </div>
                </div>
              );
          ...
          {/* X-axis labels: show every 7th day + last */}
          <div className="mt-2 flex">
            {days.map(([key], i) => (
              <div key={key} className="flex-1 text-center text-[9px] text-faint">
                {i % 7 === 0 || i === days.length - 1 ? shortDate(key) : ""}
              </div>
            ))}
          </div>
        </div>
              ...
              {byTemplate.size === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-[13px] text-muted">
                    No campaign sends yet — template performance will appear here once a campaign runs.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="flex flex-col">
                  ...
                  <div key={id} className="flex items-center gap-4 border-b border-border px-4 py-3 last:border-0">
                    ...
                    <div className="flex-shrink-0 text-right">
                      ...
                      <div className="text-[11px] text-faint">
                        {c.orderCount} sale{c.orderCount === 1 ? "" : "s"} · {pct}%
                      </div>
                    </div>
                  </div>
                );
            ...
            {organicRevenue > 0 && (
              <div className="flex items-center justify-between border-b border-border px-4 py-3 last:border-0">
                ...
                <span className="font-mono text-[13.5px] tabular-nums text-muted">
                  {formatCurrency(organicRevenue, currency)}
                </span>
              </div>
            )}
          </div>
